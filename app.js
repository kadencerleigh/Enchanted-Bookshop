...
function mapGenres(raw,title,desc){var t=([].concat(raw||[]).join(" ")+" "+(title||"")+" "+(desc||"")).toLowerCase(),a=[];function add(x){if(a.indexOf(x)<0)a.push(x)}
 if(/romantasy/.test(t)){add("Romantasy");add("Fantasy");add("Romance")} if(/fantasy|magic|fairy|fae|witch|dragon/.test(t))add("Fantasy"); if(/romance|love stor/.test(t))add("Romance"); if(/dark romance/.test(t))add("Dark Romance"); if(/mystery|detective/.test(t))add("Mystery"); if(/thriller|suspense/.test(t))add("Thriller"); if(/horror|ghost|haunt/.test(t))add("Horror"); if(/contemporary/.test(t))add("Contemporary"); if(/realistic fiction/.test(t))add("Realistic Fiction"); if(/historical/.test(t))add("Historical Fiction"); if(/science fiction|sci-fi|space opera/.test(t))add("Science Fiction"); if(/dystop/.test(t))add("Dystopian"); if(/paranormal|vampire|werewolf/.test(t))add("Paranormal"); if(/young adult|juvenile fiction|teen/.test(t))add("Young Adult"); if(/new adult/.test(t))add("New Adult"); if(/middle grade/.test(t))add("Middle Grade"); if(/literary fiction/.test(t))add("Literary Fiction"); if(/nonfiction|non-fiction/.test(t))add("Nonfiction"); if(/memoir|biograph/.test(t))add("Memoir/Biography"); if(/true crime/.test(t))add("True Crime"); if(/psycholog|mental health/.test(t))add("Psychology/Mental Health"); if(/poetry|poems/.test(t))add("Poetry"); if(/graphic novel|manga|comics/.test(t))add("Graphic Novel/Manga"); return a.length?a:(raw||[]).slice(0,4)}
function suggestSpice(gs,d){var t=((gs||[]).join(" ")+" "+(d||"")).toLowerCase();if(/erotic|erotica|explicit|very spicy/.test(t))return 5;if(/dark romance|steamy|spicy/.test(t))return 4;if(/romantasy|romance|romantic/.test(t))return 2;return 0}
...
function openBook(b){
 ...
 field("Title","title",b.title,true)+field("Author","author",b.author)+genreField(b.genres||[])+field("Series","series",b.series)+field("Series number","seriesNo",b.seriesNo)+
...
async function lookupISBN(){
 ...
 try{var g=await fetch("https://www.googleapis.com/books/v1/volumes?q=isbn:"+encodeURIComponent(isbn));if(g.ok){var j=await g.json();if(j.items&&j.items.length){var v=j.items[0].volumeInfo||{};found={title:v.title||"",author:(v.authors||[]).join(", "),genres:v.categories||[],description:v.description||"",cover:(v.imageLinks&&(v.imageLinks.thumbnail||v.imageLinks.smallThumbnail))||"",edition:{isbn:isbn,format:"",publisher:v.publisher||"",publicationDate:v.publishedDate||"",pages:v.pageCount||"",printing:"",special:[]}};source="Google Books"}}}catch(e){}
 if(!found)try{var o=await fetch("https://openlibrary.org/isbn/"+isbn+".json");if(o.ok){var q=await o.json();found={title:q.title||"",author:"",genres:q.subjects||[],description:q.description&&((typeof q.description==="string")?q.description:q.description.value)||"",cover:"https://covers.openlibrary.org/b/isbn/"+isbn+"-L.jpg",edition:{isbn:isbn,format:q.physical_format||"",publisher:(q.publishers||[]).join(", "),publicationDate:q.publish_date||"",pages:q.number_of_pages||"",printing:"",special:[]}};source="Open Library"}}catch(e){}
...
function prefill(f){var gs=mapGenres(f.genres||[],f.title||"",f.description||""),ss=suggestSpice(gs,f.description||"");openBook({id:"",workId:"",title:f.title||"",author:f.author||"",genres:gs,series:"",seriesNo:"",status:"want-to-read",owned:true,wantOwn:false,rating:0,favorite:false,spice:ss,spiceSuggested:true,spiceConfirmed:false,finishedDate:"",cover:f.cover||"",edition:f.edition,notes:(ss?"Suggested spice "+ss+"/5 from public metadata — confirm/edit anytime.":"Spice could not be reliably estimated from public metadata — confirm/edit anytime.")})}
function closeModal(){stopScanner();el("#modal").classList.add("hidden")}
function exportJSON(){var blob=new Blob([JSON.stringify({app:"Enchanted Bookshop",version:"4.3",exported:now(),books:state.books,seriesCatalog:seriesCatalog,readingChallenges:getChallenges()},null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="enchanted-bookshop-v3-backup.json";document.body.appendChild(a);a.click();a.remove()}
