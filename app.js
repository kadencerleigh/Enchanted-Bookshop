...
function backup(){
 ...
 return '<section class="backup-hero"><div><div class="eyebrow">💾 V4.22 • BACKUP CENTER 2.0</div><h1 class="title">Protect your bookshop.</h1><p class="sub">See what is protected, make a complete portable backup, and restore only after reviewing what the file contains.</p></div><div class="backup-shield">🛡️</div></section>'+ 
...
function backupPayload(){return{app:"Enchanted Bookshop",version:"4.22",exported:now(),books:state.books,seriesCatalog:seriesCatalog,readingChallenges:getChallenges(),workIntelligence:workIntelligence,smartShelves:smartShelfList(),preferences:{homeDashboard:homeDashSettings(),libraryView:libraryView()}}}
function exportJSON(){var payload=backupPayload(),blob=new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),a=document.createElement("a"),stamp=new Date().toISOString().slice(0,10);a.href=URL.createObjectURL(blob);a.download="enchanted-bookshop-v4-22-backup-"+stamp+".json";document.body.appendChild(a);a.click();a.remove();setBackupMeta({lastExport:payload.exported,version:"4.22"});setTimeout(function(){if(state.view==="backup")render()},150)}
function inspectRestore(file){if(!file)return;var r=new FileReader();r.onload=function(){var box=el("#restorePreview");try{var x=JSON.parse(r.result);if(!x||!Array.isArray(x.books))throw Error("Invalid backup");var journals=0;x.books.forEach(function(b){journals+=[].concat(b.readingJournal||[]).length});var sc=Array.isArray(x.seriesCatalog)?x.seriesCatalog.length:0,ss=Array.isArray(x.smartShelves)?x.smartShelves.length:0,wi=x.workIntelligence&&typeof x.workIntelligence==="object"?Object.keys(x.workIntelligence).length:0;box.innerHTML='<div class="restore-card"><b>✨ Backup looks readable</b><div class="restore-stats"><span>📚 '+x.books.length+' books</span><span>🧩 '+sc+' series</span><span>🕯️ '+journals+' journal entries</span><span>🧠 '+wi+' work records</span><span>🔍 '+ss+' smart shelves</span></div><div class="tiny muted">Backup version: '+esc(x.version||"Unknown")+(x.exported?' • Exported '+esc(new Date(x.exported).toLocaleString()):'')+'</div><div class="actions"><button class="danger" id="confirmRestore">Restore this backup</button><button class="pill" id="cancelRestore">Cancel</button></div><div class="tiny"><b>Restore replaces the local Bookshop data represented in this file.</b> Nothing changes until you press Restore this backup.</div></div>';el("#confirmRestore").onclick=function(){if(!confirm("Restore this backup now? Your current local Bookshop data will be replaced by the backup contents."))return;applyRestore(x)};el("#cancelRestore").onclick=function(){box.innerHTML='<span class="muted">Restore cancelled. Nothing was changed.</span>';el("#restoreFile").value=""}}catch(e){box.innerHTML='<div class="syncstatus bad">⚠️ This file does not look like a readable Enchanted Bookshop backup. Nothing was changed.</div>'}};r.readAsText(file)}
