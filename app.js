...
function journalSessions(b){return [].concat(b.readingJournal||[]).slice().sort(function(a,c){return String(c.date||"").localeCompare(String(a.date||""))})}
function readingJournal2(){
 ...
 function sessionCard(x){var s=x.session,b=x.book;return '<article class="reading-entry"><div class="reading-entry-top"><div><div class="eyebrow">'+esc(s.date||'Reading session')+'</div><h3>'+esc(b.title)+'</h3></div><span class="reading-mood">'+esc(s.mood||'📖')+'</span></div><div class="reading-entry-meta">'+(s.page?'<span>📄 Page '+esc(s.page)+'</span>':'')+(s.percent?'<span>✨ '+esc(s.percent)+'%</span>':'')+(s.minutes?'<span>⏱️ '+esc(s.minutes)+' min</span>':'')+'</div>'+(s.reaction?'<blockquote>“'+esc(s.reaction)+'”</blockquote>':'')+(s.quote?'<div class="journal-quote">💬 '+esc(s.quote)+'</div>':'')+(s.notes?'<p>'+esc(s.notes)+'</p>':'')+'</article>'}
 return '<div class="journal-hero"><div class="eyebrow">📖 V4.15.1 • READING JOURNAL 2.0</div><h1 class="title">My Reading Journal</h1><p class="sub">Track the reading, not just the books — progress, sessions, reactions, quotes, rereads, and the little thoughts you want to remember.</p></div><div class="journal-stats"><div><b>'+active.length+'</b><span>Reading now</span></div><div><b>'+sessions.length+'</b><span>Sessions logged</span></div><div><b>'+finished.length+'</b><span>Finished books</span></div></div><section class="journal-section"><div class="journal-heading"><div><div class="eyebrow">🌙 ON THE NIGHTSTAND</div><h2>Currently reading</h2></div></div><div class="journal-books">'+(active.length?active.map(activeCard).join(''):'<div class="empty">Nothing is marked Currently Reading yet. Open a book and change its reading status when you start. ✨</div>')+'</div></section><section class="journal-section"><div class="journal-heading"><div><div class="eyebrow">🕯️ READING MEMORIES</div><h2>Recent journal entries</h2></div></div><div class="reading-timeline">'+(sessions.length?sessions.slice(0,30).map(sessionCard).join(''):'<div class="empty">Your journal is waiting for its first reading session. 📖</div>')+'</div></section>'
}
function openReadingSession(b){
 var rp=b.readingProgress||{},today=dateOnly(),totalPages=parseInt(b.edition&&b.edition.pages,10),hasPages=Number.isFinite(totalPages)&&totalPages>0;
 var percentField=field('Percent complete','sessionPercent',rp.percent||'');
 ...
 el('#modalBody').innerHTML='<div class="eyebrow">📖 READING JOURNAL 2.0</div><h1 class="title">Log your reading</h1><p class="sub"><b>'+esc(b.title)+'</b> • '+esc(b.author||'')+(hasPages?' • '+esc(totalPages)+' pages':'')+'</p><form id="readingSessionForm"><div class="form">'+field('Date','sessionDate',today,true)+field('Page','sessionPage',rp.page||'')+percentField+field('Minutes read','sessionMinutes','')+selectField('Reading mood','sessionMood',['📖 Cozy','😍 Obsessed','😳 WHAT','😭 Emotional','😡 Furious','🤔 Thinking','😴 Sleepy'], '📖 Cozy')+'<div class="field full"><label>Immediate reaction</label><textarea name="sessionReaction" placeholder="WHAT JUST HAPPENED?!"></textarea></div><div class="field full"><label>Favorite quote from this session</label><textarea name="sessionQuote" placeholder="A line you want to remember..."></textarea></div><div class="field full"><label>Notes / theories / thoughts</label><textarea name="sessionNotes" placeholder="Predictions, character thoughts, feelings, theories..."></textarea></div></div><div class="actions"><button class="primary" type="submit">🕯️ Save journal entry</button></div></form>';
 el('#modal').classList.remove('hidden');
 ...
 form.onsubmit=function(ev){ev.preventDefault();var f=new FormData(ev.target),entry={id:'r_'+Date.now().toString(36),date:f.get('sessionDate'),page:f.get('sessionPage'),percent:f.get('sessionPercent'),minutes:f.get('sessionMinutes'),mood:f.get('sessionMood'),reaction:f.get('sessionReaction'),quote:f.get('sessionQuote'),notes:f.get('sessionNotes'),createdAt:now()};b.readingJournal=[].concat(b.readingJournal||[]);b.readingJournal.push(entry);b.readingProgress={page:entry.page,percent:entry.percent,updatedAt:now()};b.updatedAt=now();save();closeModal();state.view='journal';render();autoSyncMaybe()}
}
function render(){
...
function exportJSON(){var blob=new Blob([JSON.stringify({app:"Enchanted Bookshop",version:"4.15",exported:now(),books:state.books,seriesCatalog:seriesCatalog,readingChallenges:getChallenges(),workIntelligence:workIntelligence},null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="enchanted-bookshop-v4-15-backup.json";document.body.appendChild(a);a.click();a.remove()}
