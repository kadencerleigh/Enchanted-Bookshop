...
function cleanSeriesResults(items){var cleaned=uniqueSeriesCandidates(items).filter(function(x){return !looksLikeEditionBundle(x.title)});var numbered=cleaned.filter(function(x){return String(x.number||"").trim()});if(numbered.length>=2)return cleaned.filter(function(x){return String(x.number||"").trim()||x.type==="Novella / Extra"});return cleaned}
async function findSeriesLineup(name){
 el("#modalBody").innerHTML='<div class="eyebrow">Series Brain™ • V4.7.1</div><h1 class="title">Finding '+esc(name)+'</h1><div class="empty">🔮 Searching by series name, known author, and public series metadata…</div>';el("#modal").classList.remove("hidden");
 ...
 var known=local.map(function(b){return{number:b.seriesNo||"",title:b.title,type:"Main novel",source:"Your library"}}),results=cleanSeriesResults(google.concat(ol,known));
