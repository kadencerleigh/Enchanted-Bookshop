...
async function googleWorkSearch(title,author,isbn){try{var q='intitle:"'+title+'"'+(author?' inauthor:"'+author+'"':'');var g=await fetch("https://www.googleapis.com/books/v1/volumes?q="+encodeURIComponent(q)+"&maxResults=20");if(!g.ok)return null;var j=await g.json(),items=j.items||[],best=null,bestScore=-1,claims={};items.forEach(function(it){var v=it.volumeInfo||{},va=(v.authors||[]).join(", "),ts=similarity(title,v.title||""),as=author?similarity(author,va):6,score=ts+as+(v.description?3:0)+(v.categories&&v.categories.length?2:0);if(ts>=6&&as>=4){var blob=[v.title||"",v.subtitle||"",v.description||"",(v.categories||[]).join(" ")].join(" ").replace(/<[^>]+>/g," "),ser=inferSeries(v.title||"",blob);if(ser.series)addSeriesClaim(claims,ser.series,ser.seriesNo,7+(ts>=9?2:0)+(as>=7?1:0),"Google Books work editions",title,author);var p=/(?:book|volume)\s*(\d+(?:\.\d+)?)\s+(?:of|in)\s+(?:the\s+)?([^.;()]{3,70}?)(?:\s+series)?(?:[.;()]|$)/ig,m;while((m=p.exec(blob)))addSeriesClaim(claims,m[2],m[1],7,"Google Books work editions",title,author)}var direct=inferSeries(v.title||"",v.description||"");if(direct.series)score+=3;if(score>bestScore){bestScore=score;best=v}});if(!best)return null;var ser=inferSeries(best.title||"",best.description||""),bridge=bestSeriesClaim(claims);if(bridge){ser.series=bridge.series;ser.seriesNo=bridge.seriesNo||ser.seriesNo||""}return{title:best.title||title||"",author:(best.authors||[]).join(", "),genres:best.categories||[],description:best.description||"",cover:(best.imageLinks&&(best.imageLinks.thumbnail||best.imageLinks.smallThumbnail))||"",series:ser.series,seriesNo:ser.seriesNo,seriesSuggested:!!bridge,seriesConfidence:bridge?(bridge.confidence||"possible"):"",seriesSource:bridge?(bridge.source||"Google Books work editions"):"",edition:{isbn:isbn,format:"",publisher:"",publicationDate:"",pages:"",printing:"",special:[]}}}catch(e){return null}}
...
async function enrichBook(found,isbn){
 ...
 if((!out.author||!(out.genres||[]).length||!out.description)&&out.title){out=mergeWorkSignals(out,await googleWorkSearch(out.title,out.author||"",isbn))}
