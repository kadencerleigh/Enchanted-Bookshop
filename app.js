...
function readingJournal2(){
 ...
 return '<div class="journal-hero"><div class="eyebrow">📖 V4.15 • READING JOURNAL 2.0</div><h1 class="title">My Reading Journal</h1><p class="sub">Track the reading, not just the books — progress, sessions, reactions, quotes, rereads, and the little thoughts you want to remember.</p></div><div class="journal-stats"><div><b>'+active.length+'</b><span>Reading now</span></div><div><b>'+sessions.length+'</b><span>Sessions logged</span></div><div><b>'+finished.length+'</b><span>Finished books</span></div></div><section class="journal-section"><div class="journal-heading"><div><div class="eyebrow">🌙 ON THE NIGHTSTAND</div><h2>Currently reading</h2></div></div><div class="journal-books">'+(active.length?active.map(activeCard).join(''):'<div class="empty">Nothing is marked Currently Reading yet. Open a book and change its reading status when you start. ✨</div>')+'</div></section><section class="journal-section"><div class="journal-heading"><div><div class="eyebrow">🕯️ READING MEMORIES</div><h2>Recent journal entries</h2></div></div><div class="reading-timeline">'+(sessions.length?sessions.slice(0,30).map(sessionCard).join(''):'<div class="empty">Your journal is waiting for its first reading session. 📖</div>')+'</div></section>'
}
function openReadingSession(b){
 var rp=b.readingProgress||{},today=dateOnly();
 el('#modalBody').innerHTML='<div class="eyebrow">📖 READING JOURNAL 2.0</div><h1 class="title">Log your reading</h1><p class="sub"><b>'+esc(b.title)+'</b> • '+esc(b.author||'')+'</p><form id="readingSessionForm"><div class="form">'+field('Date','sessionDate',today,true)+field('Page','sessionPage',rp.page||'')+field('Percent complete','sessionPercent',rp.percent||'')+field('Minutes read','sessionMinutes','')+selectField('Reading mood','sessionMood',['📖 Cozy','😍 Obsessed','😳 WHAT','😭 Emotional','😡 Furious','🤔 Thinking','😴 Sleepy'], '📖 Cozy')+'<div class="field full"><label>Immediate reaction</label><textarea name="sessionReaction" placeholder="WHAT JUST HAPPENED?!"></textarea></div><div class="field full"><label>Favorite quote from this session</label><textarea name="sessionQuote" placeholder="A line you want to remember..."></textarea></div><div class="field full"><label>Notes / theories / thoughts</label><textarea name="sessionNotes" placeholder="Predictions, character thoughts, feelings, theories..."></textarea></div></div><div class="actions"><button class="primary" type="submit">🕯️ Save journal entry</button></div></form>';
 el('#modal').classList.remove('hidden');el('#readingSessionForm').onsubmit=function(ev){ev.preventDefault();var f=new FormData(ev.target),entry={id:'r_'+Date.now().toString(36),date:f.get('sessionDate'),page:f.get('sessionPage'),percent:f.get('sessionPercent'),minutes:f.get('sessionMinutes'),mood:f.get('sessionMood'),reaction:f.get('sessionReaction'),quote:f.get('sessionQuote'),notes:f.get('sessionNotes'),createdAt:now()};b.readingJournal=[].concat(b.readingJournal||[]);b.readingJournal.push(entry);b.readingProgress={page:entry.page,percent:entry.percent,updatedAt:now()};b.updatedAt=now();save();closeModal();state.view='journal';render();autoSyncMaybe()}
}
function render(){
