...
async function cachedFetchJSON(url){var hit=getCachedJSON(url);if(hit!==null)return{ok:true,status:200,data:hit,fromCache:true};try{var r=await fetch(url);if(!r.ok)return{ok:false,status:r.status,data:null,fromCache:false};var j=await r.json();setCachedJSON(url,j);return{ok:true,status:r.status,data:j,fromCache:false}}catch(e){return{ok:false,status:0,data:null,fromCache:false}}}
...
function intelligenceFor(f){var k=knownBook(f.title||"",f.author||""),learned=getWorkIntelligence(f.title||"",f.author||""),cat=seriesFromCatalog(f.title||""),ser=(k&&k.series)?{series:k.series,seriesNo:k.seriesNo||""}:cat.series?cat:inferSeries(f.title||"",f.description||"");var gs=mapGenres(f.genres||[],f.title||"",f.description||"",f.author||""),tags=(learned&&learned.tags&&learned.tags.length)?learned.tags.slice():inferTags(f.genres||[],f.title||"",f.description||"",f.author||""),ss=suggestSpice(gs,f.description||"",f.title||"",f.author||""),conf=spiceConfidence(gs,f.description||"",ss,f.title||"",f.author||"");if(learned&&learned.spice!=null){ss=+learned.spice;conf="confirmed by you"}return{genres:gs,tags:tags,series:ser.series||f.series||"",seriesNo:ser.seriesNo||f.seriesNo||"",spice:ss,spiceConfidence:conf,source:learned?"Confirmed in your Bookshop Brain":(k?(k.source||"Enchanted Bookshop knowledge"):"Public book metadata"),workBrain:!!learned}}
...
function exportJSON(){var blob=new Blob([JSON.stringify({app:"Enchanted Bookshop",version:"4.7.8",exported:now(),books:state.books,seriesCatalog:seriesCatalog,readingChallenges:getChallenges(),workIntelligence:workIntelligence},null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="enchanted-bookshop-v4-7-8-backup.json";document.body.appendChild(a);a.click();a.remove()}
