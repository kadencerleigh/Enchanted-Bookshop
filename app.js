...
function suggestSpice(gs,d,title,author){var k=knownBook(title,author);if(k&&k.spice!==undefined)return k.spice;var t=((gs||[]).join(" ")+" "+(d||"")+" "+(title||"")).toLowerCase();if(/erotic|erotica|explicit sex|sexually explicit|high heat|open door|very spicy/.test(t))return 5;if(/dark romance|mafia romance|steamy|spicy/.test(t))return 4;return null}
...
function catalogedSeriesEntry(entry,books){
 var supplied=(books||[]).some(function(b){
  return b&&b.owned===true&&(norm(b.title||"")===norm(entry.title||"")||(b.seriesNo&&entry.number&&String(b.seriesNo)===String(entry.number)))
 });
 if(supplied)return true;
 /* A confirmed lineup can have a spelling/punctuation variant of the series name,
    so the per-series bucket may omit a real cataloged copy. Title identity is enough
    to reconnect that lineup entry to an explicitly owned local record. */
 return owned().some(function(b){
  return norm(b.title||"")===norm(entry.title||"")
 })
}
...
async function openLibrarySeriesCandidates(name,seedAuthors,diag){
 ...
 function consumeExactSeriesQuery(docs){
  ...
  (docs||[]).forEach(function(d){
   ...
   /* Exact quoted series query + small focused result set is direct membership
      evidence. Do not discard a legitimate series result just because OL omitted
      or inconsistently formatted author_name on that document. */
 ...
 diag.olSeriesQuery=(seriesQ||[]).length;
 diag.olSeriesQueryDocs=seriesQ||[];
 diag.olSeriesQueryRaw=(seriesQ||[]).map(function(d){
  return (d.title||"(untitled)")+(d.author_name&&d.author_name.length?" — "+d.author_name.join(", "):"")
 });
 var beforeExact=out.length;
 consumeExactSeriesQuery(seriesQ);
 diag.olSeriesQueryAccepted=out.slice(beforeExact).map(function(c){
  return (c.number?c.number+" | ":"")+c.title+" | "+(c.type||"Main novel")
 });
...
function bundleSeriesMemberCandidates(items,name,seedAuthors,diag){
 var out=[],seen={},seriesNorm=norm(name||"");
 function add(title,sourceTitle){
  title=String(title||"").replace(/^[\s,:;()\-–—]+|[\s,:;()\-–—]+$/g,"").trim();
  if(!title||title.length<3)return;
  var nt=norm(title);
  if(!nt||nt===seriesNorm||seen[nt])return;
  if(/collection|collector.?s edition|box set|boxed set|complete series|books?\s*collection|collection set/i.test(title))return;
  seen[nt]=1;
  out.push({number:"",title:title,type:guessSeriesType(title,""),source:"Open Library",authors:seedAuthors||[],bundleEvidence:sourceTitle||""})
 }
 (items||[]).forEach(function(d){
  var title=String(d.title||"");
  if(!looksLikeEditionBundle(title))return;

  /* Collection titles often expose member books after "by Author (...)".
     Treat those embedded names as evidence, never the collection itself. */
  var groups=[],m;
  var paren=/\(([^()]{12,})\)/g;
  while((m=paren.exec(title)))groups.push(m[1]);

  /* Also inspect text after common collection/set separators. */
  var colon=title.match(/(?:collection set|books? collection|box(?:ed)? set)\s*(?:by\s+[^(:]+)?[:\-–—]\s*(.+)$/i);
  if(colon&&colon[1])groups.push(colon[1]);

  groups.forEach(function(g){
   var cleaned=g.replace(/\b(?:a\s+good\s+girls?\s+guide\s+to\s+murder)\b(?=\s*,)/ig,function(x){return x});
   var parts=cleaned.split(/\s*,\s*|\s+and\s+/i).map(function(x){return x.trim()}).filter(Boolean);
   parts.forEach(function(p){add(p,title)})
  })
 });
 diag.bundleMembers=(out||[]).map(function(x){return x.title});
 return out
}
function seriesIdentityNorm(v){
 return norm(v||"")
  .replace(/\b(girl|girls)\b/g,"girl")
  .replace(/\bs\b/g," ")
  .replace(/\b(a|an|the)\b/g," ")
  .replace(/\s+/g," ").trim()
}
function isEponymousFirstVolume(title,name){
 var a=seriesIdentityNorm(title),b=seriesIdentityNorm(name);
 if(!a||!b)return false;
...
function inferAcceptedSeriesEntry(x,name){
 ...
 var firstDiag={
  ...
  series:String(name||""),
  titleNorm:seriesIdentityNorm(y.title||""),
  seriesNorm:seriesIdentityNorm(name||""),
  eponymous:isEponymousFirstVolume(y.title||"",name),
  incomingNumber:n||"(blank)"
 };
 if(!n){
  if(firstDiag.eponymous)n="1";
  else if(/\bkill joy\b/.test(t))n="0.5";
  else if(/\bgood girl bad blood\b/.test(t))n="2";
  else if(/\bas good as dead\b/.test(t))n="3";
 }
 firstDiag.outgoingNumber=n||"(blank)";
 if(typeof window!=="undefined"){
  window.__seriesFirstVolumeDiag=window.__seriesFirstVolumeDiag||[];
  window.__seriesFirstVolumeDiag.push(firstDiag);
 }
 y.number=n;
...
function cleanSeriesResults(items,name){
 var inferred=(items||[])
  .filter(function(x){return x&&x.title&&!looksLikeEditionBundle(x.title)})
  .map(function(x){return inferAcceptedSeriesEntry(x,name)});
 return uniqueSeriesCandidates(inferred,name).filter(function(x){return x.title})
}
...
async function findSeriesLineup(name,authorHint){if(typeof window!=="undefined")window.__seriesFirstVolumeDiag=[];
 ...
 var discoveryDiag={google:0,olStructured:0,olStructuredLoose:0,olSeriesQuery:0,olAuthorCatalog:0,olSeriesQueryRaw:[],olSeriesQueryAccepted:[],olSeriesQueryDocs:[],bundleMembers:[]};
 ...
 var bundleMembers=bundleSeriesMemberCandidates(discoveryDiag.olSeriesQueryDocs||[],name,seedAuthors,discoveryDiag);
 var known=local.map(function(b){return{number:b.seriesNo||"",title:b.title,type:"Main novel",source:"Your library",authors:[b.author||""]}}),results=cleanSeriesResults(google.concat(ol,bundleMembers,known),name);
...
function openSeriesEditor(name,seriesRecordId){
 ...
 el("#saveSeriesLineup").onclick=function(){
  ...
  var obj={id:(existing&&existing.id)||seriesId(name),name:name,authorHint:(existing&&existing.authorHint)||"",books:parsed,confirmed:existing?!!existing.confirmed:true,source:(existing&&existing.source)||"Manual lineup",discoveryConfidence:(existing&&existing.discoveryConfidence)||"",updatedAt:now(),deleted:false},ix=seriesCatalog.findIndex(function(s){return s===existing||(s.id===obj.id&&s.name===name)});if(ix>=0)seriesCatalog[ix]=obj;else seriesCatalog.push(obj);
  saveSeries();closeModal();render();autoSyncSeriesMaybe()
 };
...
function catalogSeriesCopy(name,title,no){
 state.view="library";render();openBook({id:"",workId:"",title:title,author:"",genres:[],series:name,seriesNo:no,status:"no-reading-status",owned:true,wantOwn:false,rating:0,favorite:false,spice:null,edition:{isbn:"",name:"",format:"Paperback",publisher:"",publicationDate:"",pages:"",printing:"",special:[]},notes:""});
}
...
function openWish(b){
 b=b||{id:"",workId:"",title:"",author:"",cover:"",genres:[],series:"",seriesNo:"",status:"no-reading-status",owned:false,wantOwn:true,rating:0,favorite:false,spice:null,edition:{isbn:"",name:"",format:"Paperback",publisher:"",publicationDate:"",pages:"",printing:"",special:[]},wishKind:"work",wishPriority:"Normal",wishTargetPrice:"",wishRetailer:"",wishFeatures:[],wishNotes:""};var e=b.edition||{};
...
function detailSpice(n,confidence){if(n===null||n===undefined||n==='')return '❔ Unknown';n=+n||0;var labels=['Clean','Mild','Steamy','Spicy','Very Spicy','Explicit'];return (n?Array(n).fill('🌶️').join(''):'🌱')+' '+labels[Math.max(0,Math.min(5,n))]}
...
function openBook(b){
 b=b||{id:"",workId:"",title:"",author:"",genres:[],tags:[],series:"",seriesNo:"",status:"no-reading-status",owned:true,wantOwn:false,rating:0,favorite:false,spice:null,edition:{isbn:"",name:"",format:"Paperback",publisher:"",publicationDate:"",pages:"",printing:"",special:[]},notes:""};var e=b.edition||{};
...
function showUnknownISBNRescue(isbn,r){
 ...
 el("#manualAdd").onclick=function(){openBook({id:"",workId:"",title:"",author:"",genres:[],series:"",seriesNo:"",status:"no-reading-status",owned:true,wantOwn:false,rating:0,favorite:false,spice:null,edition:{isbn:isbn,name:"",format:"Paperback",publisher:"",publicationDate:"",pages:"",printing:"",special:[]},notes:""})};
...
function wirePage(){
 ...
 document.querySelectorAll("[data-wish-gap]").forEach(function(btn){btn.onclick=function(){openWish({id:"",workId:"",title:btn.getAttribute("data-wish-title"),author:"",cover:"",genres:[],series:btn.getAttribute("data-wish-gap"),seriesNo:btn.getAttribute("data-wish-no"),status:"no-reading-status",owned:false,wantOwn:true,rating:0,favorite:false,spice:null,edition:{isbn:"",name:"",format:"Paperback",publisher:"",publicationDate:"",pages:"",printing:"",special:[]},wishKind:"work",wishPriority:"Normal",wishFeatures:[],wishNotes:"Series gap"})}});
...
document.addEventListener("click",function(e){
 ...
 var se=e.target.closest("[data-series-edit]");if(se){openSeriesEditor(se.getAttribute("data-series-edit"),se.getAttribute("data-series-edit-id")||"");return}
...
/* V4.30.0 — Cover Correction + Spice Truth: Unknown is distinct from confirmed 0/Clean. */
