...
function inferSeries(title,desc){var t=((title||"")+" "+(desc||"")).replace(/<[^>]+>/g," "),m;
 m=t.match(/(?:book|volume)\s*(?:#|no\.?\s*)?(\d+(?:\.\d+)?)\s+(?:of|in)\s+(?:the\s+)?([^.;:()]{3,60}?)(?:\s+series)?(?:[.;:()]|$)/i);if(m)return{series:m[2].trim().replace(/\s+series$/i,""),seriesNo:m[1]};
 m=t.match(/(?:the\s+)?([^.;:()]{3,60}?)\s+series[,;:]?\s*(?:book|volume)\s*(?:#|no\.?\s*)?(\d+(?:\.\d+)?)/i);if(m)return{series:m[1].trim(),seriesNo:m[2]};
 m=t.match(/(?:first|1st)\s+(?:full-length\s+)?book\s+in\s+(?:the\s+)?([^.;:()]{3,60}?)\s+series/i);if(m)return{series:m[1].trim(),seriesNo:"1"};
 m=t.match(/(?:first|1st)\s+book\s+in\s+(?:the\s+)?([^.;:()]{3,60}?)(?:\s+series)?(?:[.;:()]|$)/i);if(m)return{series:m[1].trim().replace(/\s+series$/i,""),seriesNo:"1"};
...
async function openLibraryWorkSearch(title,author,isbn){try{var qs="title="+encodeURIComponent(title||"")+(author?"&author="+encodeURIComponent(author):"")+"&limit=12&fields=title,author_name,subject,first_publish_year,cover_i,key";var r=await fetch("https://openlibrary.org/search.json?"+qs);if(!r.ok)return null;var j=await r.json(),docs=j.docs||[],d=null,best=-1;docs.forEach(function(x){var sc=similarity(title,x.title||"")+similarity(author,(x.author_name||[]).join(" "))+(x.subject&&x.subject.length?2:0);if(sc>best){best=sc;d=x}});if(!d)return null;var desc="";if(d.key&&/^\/works\//.test(d.key)){try{var wr=await fetch("https://openlibrary.org"+d.key+".json");if(wr.ok){var w=await wr.json();desc=w.description&&((typeof w.description==="string")?w.description:w.description.value)||""}}catch(e){}}var ser=inferSeries(d.title||title,desc);return{title:d.title||title||"",author:(d.author_name||[]).join(", "),genres:d.subject||[],description:desc,cover:d.cover_i?("https://covers.openlibrary.org/b/id/"+d.cover_i+"-L.jpg"):"",series:ser.series,seriesNo:ser.seriesNo,edition:{isbn:isbn,format:"",publisher:"",publicationDate:"",pages:"",printing:"",special:[]}}}catch(e){return null}}
...
function exportJSON(){var blob=new Blob([JSON.stringify({app:"Enchanted Bookshop",version:"4.6",exported:now(),books:state.books,seriesCatalog:seriesCatalog,readingChallenges:getChallenges()},null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="enchanted-bookshop-v4-6-backup.json";document.body.appendChild(a);a.click();a.remove()}
