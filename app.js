...
function suggestSpice(gs,d,title){var t=((gs||[]).join(" ")+" "+(d||"")+" "+(title||"")).toLowerCase();if(/erotic|erotica|explicit sex|sexually explicit|high heat|open door|very spicy/.test(t))return 5;if(/dark romance|mafia romance|steamy|spicy|sensual|arranged marriage|enemies[- ]to[- ]lovers/.test(t))return 4;if(/romantasy|romance|romantic/.test(t))return 2;return 0}
function spiceConfidence(gs,d,score){var t=((gs||[]).join(" ")+" "+(d||"")).toLowerCase();if(!score)return"low";if(/erotic|explicit|high heat|open door|dark romance|mafia romance|steamy|spicy/.test(t))return"medium";return"low"}
...
function prefill(f){var gs=mapGenres(f.genres||[],f.title||"",f.description||""),ss=suggestSpice(gs,f.description||"",f.title||""),conf=spiceConfidence(gs,f.description||"",ss);openBook({id:"",workId:"",title:f.title||"",author:f.author||"",genres:gs,series:f.series||"",seriesNo:f.seriesNo||"",status:"want-to-read",owned:true,wantOwn:false,rating:0,favorite:false,spice:ss,spiceSuggested:true,spiceConfirmed:false,finishedDate:"",cover:f.cover||"",edition:f.edition,notes:(ss?"Suggested spice "+ss+"/5 ("+conf+" confidence) from book metadata — confirm/edit anytime.":"No reliable spice signal found in book metadata — confirm/edit anytime.")})}
function closeModal(){stopScanner();el("#modal").classList.add("hidden")}
function exportJSON(){var blob=new Blob([JSON.stringify({app:"Enchanted Bookshop",version:"4.4",exported:now(),books:state.books,seriesCatalog:seriesCatalog,readingChallenges:getChallenges()},null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="enchanted-bookshop-v3-backup.json";document.body.appendChild(a);a.click();a.remove()}
